...
func init() {
	...
	// Ansible flags
	rootCmd.Flags().StringSlice("ansible-doc-fragments", []string{}, "comma-separated list of doc fragments to add to ansible modules")
	rootCmd.Flags().StringSlice("ansible-info-doc-fragments", []string{}, "comma-separated list of doc fragments to add to ansible _info modules")
	rootCmd.Flags().StringSlice("ansible-authors", []string{}, "comma-separated list of authors to add to ansible module documentation")
	rootCmd.Flags().String("ansible-module-prefix", "", "prefix to prepend to all generated resources")

	// Path flags
	rootCmd.Flags().StringP("output", "o", "", "path to write autogenerated files (default: './output' if mmv1-config.yaml, './' otherwise)")
	rootCmd.Flags().StringP("overlay", "O", "overlay", "overlay directory (layout: products/, templates/) merged over the cloned mmv1 root")
	rootCmd.Flags().StringP("templates", "T", "templates", "path to the ansible-mmv1 templates directory")
	...
	mustBindPFlag("templates", "templates")
	mustBindPFlag("overwrite", "overwrite")
	mustBindPFlag("min-version", "min-version")
	mustBindPFlag("ansible.doc-fragments", "ansible-doc-fragments")
	mustBindPFlag("ansible.info-doc-fragments", "ansible-info-doc-fragments")
	mustBindPFlag("ansible.authors", "ansible-authors")
	mustBindPFlag("ansible.module-prefix", "ansible-module-prefix")
}
...
func initConfig() {
	...
	if configFile == "" {
		// No --config flag: walk the candidate list and use the first file that exists.
		for _, candidate := range configCandidates {
			if _, err := os.Stat(candidate); err == nil {
				viper.SetConfigFile(candidate)
				configFileBaseName = candidate
				break
			}
		}
	} else {
		// User supplied an explicit path - use it directly and record the base name
		// so the output heuristic can normalise absolute paths like $(pwd)/mmv1-config.yaml.
...
func runGenerate(cmd *cobra.Command, args []string) {
	...
	// Get ansible configuration from viper (config file + flags merged)
	ansibleAuthors := viper.GetStringSlice("ansible.authors")
	ansibleDocFragments := viper.GetStringSlice("ansible.doc-fragments")
	ansibleInfoDocFragments := viper.GetStringSlice("ansible.info-doc-fragments")
	ansibleModulePrefix := viper.GetString("ansible.module-prefix")
	...
	for _, pKey := range api.ProductKeys(loader) {
		...
		apiProd := api.WrapProduct(mmv1Product, mmv1Root, ansibleModulePrefix)
